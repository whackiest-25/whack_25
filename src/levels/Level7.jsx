...
import {
    Rocket, AlertTriangle, CheckCircle, Flame,
    RotateCcw, RefreshCw, Crosshair, Activity,
    ArrowDownToLine, Zap, Globe, Gauge, XCircle, ArrowLeftRight, Anchor,
    Info, Target, BookOpen, Medal, Lightbulb, AlertOctagon, ArrowLeft
} from 'lucide-react';
...
const MRD_CONTENT = {
    ...
    failure_tips: {
        "FUEL EXHAUSTED": {
            ...
            analysis: "The fuel tanks ran dry before achieving orbital velocity. This usually happens from burning too early when far from Mars, or maintaining a poor attitude angle.",
            correction: "Only burn when close to Mars (distance < 1000 km). Ensure attitude is near 0° for maximum braking efficiency. Each degree of misalignment wastes precious fuel."
        },
        "CRITICAL DECAY": {
            title: "VELOCITY TOO LOW - SURFACE IMPACT",
            analysis: "The spacecraft slowed below 3.0 km/s, the minimum velocity to maintain orbit. Mars gravity pulled the spacecraft down, resulting in atmospheric reentry and impact.",
            correction: "Monitor velocity carefully. Release the burn immediately if approaching 3.0 km/s. It's better to escape than crash."
        },
        "GRAVITY ESCAPE": {
            title: "FLYBY - MISSED MARS CAPTURE",
            analysis: "The spacecraft failed to slow down enough during the approach window and flew past Mars into deep space. Insufficient braking = permanent mission loss.",
            correction: "Start burning earlier and hold longer. You need to reduce from 5.8 km/s down to 4.0-4.4 km/s during the close approach window."
        },
        "VELOCITY HIGH": {
            title: "INSUFFICIENT BRAKING - ESCAPE TRAJECTORY",
            analysis: "Final velocity above 4.4 km/s exceeds Mars escape velocity. The spacecraft will continue on a hyperbolic trajectory away from Mars.",
            correction: "You didn't burn long enough. Hold IGNITE LAM longer to slow down further. Watch the velocity bar and aim for the green success zone."
        },
        "VELOCITY LOW": {
            title: "EXCESSIVE BRAKING - ORBITAL DECAY",
            analysis: "Final velocity below 4.0 km/s is insufficient to maintain a stable orbit. The spacecraft will gradually spiral into Mars atmosphere due to gravity.",
            correction: "You burned too long. Release IGNITE as soon as velocity enters the green zone (4.0-4.4 km/s). Precision is key."
        },
        "PREMATURE PARKING": {
            title: "PARKING ATTEMPT TOO FAR FROM MARS",
            analysis: "You attempted to park in orbit while still too far from Mars periapsis. Orbital insertion must occur at the closest approach point.",
            correction: "Wait until distance reads < 2000 km and you're passing Mars (top of screen). Only then attempt to park."
        }
    },
    success: {
        title: "MARS ORBIT CAPTURED",
        mission_impact: "Spectacular flying! MOM has successfully inserted into Mars orbit. After traveling 680 million kilometers over 300 days, we've achieved orbital capture with precision braking.",
        details: [
            {
                topic: "FLIGHT DATA",
                content: "• Mission Duration: 300 days\n• Total Distance: 680 million km\n• Final Orbit: 422 km × 76,994 km (elliptical)\n• Insertion Burn: ~24 minutes\n• Status: All systems nominal"
            },
            {
                topic: "MISSION VOCABULARY",
                content: "• MOI: Mars Orbit Insertion - the critical braking maneuver\n• LAM: Liquid Apogee Motor - main engine\n• PERIAPSIS: Closest point to Mars in orbit\n• APOAPSIS: Farthest point from Mars in orbit\n• CAPTURE WINDOW: Velocity range for stable orbit (4.0-4.4 km/s)"
            },
            {
                topic: "PERFORMANCE REPORT",
                content: "Your burn timing and attitude control were excellent. The spacecraft is now in a highly elliptical Mars orbit, perfectly positioned for the science mission phase."
            },
            {
                topic: "HISTORIC ACHIEVEMENT",
                content: "On September 24, 2014, India's Mars Orbiter Mission became the first Asian nation to reach Mars orbit, and the first nation to succeed on their very first attempt. You've just recreated this historic achievement!"
            }
        ]
    }
};
...
const DashboardButton = ({ label, onClick, disabled, variant = "neutral", active, icon: Icon, className = "", ...props }) => {
    const variants = {
...
export default function Level7MOI({ onBack, onNextLevel }) {
    ...
    const loop = useCallback(() => {
        setGameState(prev => {
            ...
            if (phase === 'approach') {
                ...
                else if (newDistX <= 100 && newDistX > -100) newMessage = "STEP 3: IGNITE! BRAKE TO GREEN ZONE";
            }
            ...
            // Orbit Params
            const orbitFactor = Math.max(0, (newVel - PHYSICS.CRASH_VELOCITY) / (PHYSICS.ESCAPE_VELOCITY - PHYSICS.CRASH_VELOCITY));
            let newApo = 0;
    ...
    let captureY = 220;
    if (Math.abs(xDist) < currentRx) captureY = orbitCy - currentRy * Math.sqrt(1 - (xDist * xDist) / (currentRx * currentRx));
                    ...
                    <div className="space-y-3">
                        <div className="bg-blue-950/30 border border-blue-800/50 p-3 rounded">
                            <div className="flex gap-2 mb-2">
                                ...
                                <span className="text-blue-300 font-pixel text-[10px]">{MRD_CONTENT.briefing.subtitle}</span>
                            </div>
                        </div>
                        ...
                        <div className="bg-amber-950/30 border border-amber-700/50 p-3 rounded">
                            <div className="flex gap-2 mb-1">
                                ...
                                <span className="font-pixel text-[9px] text-amber-400">{MRD_CONTENT.briefing.physics_tip.label}</span>
                            </div>
                            <p className="text-amber-200/90 text-xs ml-5">{MRD_CONTENT.briefing.physics_tip.text}</p>
                        </div>
                    </div>
                </Modal>
            )}
                    ...
                    <div className="space-y-3">
                        <div className="bg-red-950/40 border border-red-700/50 p-3 rounded">
                            <div className="flex gap-2 mb-2">
                                ...
                                <span className="font-pixel text-[10px] text-red-400">{MRD_CONTENT.failure_tips[failData].title}</span>
                            </div>
                        </div>
            ...
            <div className="w-full max-w-5xl border-b-4 border-slate-800 pb-4 mb-6 flex justify-between items-end">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 mt-1">
                            <span className="text-[10px] bg-slate-800 px-2 py-0.5 rounded text-slate-400 font-mono">PHASE: {phase.toUpperCase()}</span>
                            <span className="text-[10px] bg-slate-800 px-2 py-0.5 rounded text-slate-400 font-mono">LAM: ARMED</span>
                        </div>
                    </div>
                </div>
                <div className="flex gap-4 items-center">
                    ...
                    <div className="text-right">
                        ...
                        <div className={`font-mono text-xl ${Math.abs(distanceX) < 100 ? "text-green-400 animate-pulse" : (distanceX < 0 ? "text-red-400" : "text-white")}`}>
                ...
                <div className="lg:col-span-8 flex flex-col gap-4">
                    <div className="relative aspect-video bg-black border-4 border-slate-700 rounded-xl overflow-hidden shadow-2xl">
                        ...
                        <div className="absolute top-4 left-4 z-20 w-36 bg-black/80 p-2 rounded border border-slate-600 backdrop-blur-sm shadow-lg">
                            <div className="text-[7px] text-green-400 font-pixel mb-1 border-b border-slate-700 pb-0.5">FLIGHT TELEMETRY</div>

                            {/* Primary: Velocity */}
                            <div className="flex justify-between items-end mb-1">
                                <div>
                                    <div className="text-[7px] text-slate-400">VELOCITY</div>
                                    <div className={`text-base font-mono font-bold ${velocity >= PHYSICS.MIN_SUCCESS_VEL && velocity <= PHYSICS.MAX_SUCCESS_VEL ? "text-green-400" : "text-white"}`}>
                                        {velocity.toFixed(2)}
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className="text-[7px] text-slate-400">TARGET</div>
                                    <div className="text-[9px] font-mono text-green-500 font-bold">{PHYSICS.MIN_SUCCESS_VEL} - {PHYSICS.MAX_SUCCESS_VEL}</div>
                                </div>
                            </div>

                            {/* Secondary: Orbit Params */}
                            <div className="space-y-0.5 bg-slate-900/50 p-1 rounded">
                                <TelemetryRow label="PERIAPSIS" value={"~" + PHYSICS.TARGET_PERIAPSIS} unit="km" color="text-green-300" />
                                <TelemetryRow
                                    label="APOAPSIS"
                                    value={apoapsis > 500000 ? "UNBOUND" : (apoapsis / 1000).toFixed(1) + "k"}
                                    unit="km"
                                    color={Math.abs(apoapsis - PHYSICS.TARGET_APOAPSIS) < 10000 ? "text-green-400" : "text-amber-400"}
                                />
                            </div>
                        </div>
                        ...
                        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 w-full text-center">
                            <div className={`inline-block px-6 py-3 font-pixel text-xs border-2 shadow-lg backdrop-blur-md rounded
                        ${phase === 'failed' ? 'bg-red-900/90 border-red-500 text-white' :
                                    phase === 'success' ? 'bg-green-900/90 border-green-500 text-white' :
                                        isBurningUI ? 'bg-amber-900/90 border-amber-500 text-white animate-pulse' :
                    ...
                    `}>
                        ...
                        <svg className="absolute inset-0 w-full h-full z-10" viewBox="0 0 800 600">
                            ...
                            <circle cx={MARS_CX} cy={MARS_CY} r={40} fill="#b91c1c" stroke="#7f1d1d" strokeWidth="4" />
                            <circle cx={MARS_CX} cy={MARS_CY} r={50} fill="none" stroke="#fca5a5" strokeWidth="1" strokeDasharray="4 4" opacity="0.3" />

                            <ellipse cx={MARS_CX} cy={MARS_CY} rx={TARGET_RX} ry={TARGET_RX} fill="none" stroke="#22c55e" strokeWidth="2" strokeDasharray="6 6" opacity="0.4" />
                            ...
                            {phase !== 'success' && phase !== 'failed' && (
                                <ellipse
                                    ...
                                    ry={currentRy}
                                    fill="none"
                                    stroke={trajectoryType === 'crash' ? '#ef4444' : (trajectoryType === 'capture' ? '#22c55e' : '#ffffff')}
                                    strokeWidth="3"
                            ...
                            {phase === 'success' && (
                                <ellipse cx={MARS_CX} cy={MARS_CY} rx={TARGET_RX} ry={TARGET_RX} fill="none" stroke="#22c55e" strokeWidth="4" />
                            )}
                ...
                <div className="lg:col-span-4 flex flex-col gap-6">
                    ...
                    <div className="bg-slate-900 border-4 border-slate-700 p-4 rounded-xl shadow-xl">
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <span className={`font-mono text-xs ${Math.abs(attitude - 0) < 5 ? "text-green-400" : "text-amber-400"}`}>
                    ...
                    <div className="bg-slate-900 border-4 border-slate-700 p-4 rounded-xl shadow-xl flex-1 flex flex-col justify-between relative overflow-hidden">
                        ...
                        <div>
                            ...
                            <div className="h-4 w-full bg-slate-800 rounded mb-4 overflow-hidden border border-slate-600 relative">
                                {/* Success Zone */}
                                <div className="absolute h-full bg-green-500/50 w-[5%] left-[50%] border-x border-green-400" style={{
                                    left: `${minVelPct}%`, width: `${successWidth}%`
                                }} />
                                <div className="absolute top-0 h-full w-1 bg-white" style={{ left: `${Math.min(100, Math.max(0, (velocity - 2) / 4 * 100))}%` }} />
                            </div>

                            <div className="flex justify-between items-end mb-4">
                                <div className="flex items-center gap-2">
                                    <Flame size={20} className={isBurningUI ? "text-orange-500 animate-bounce" : "text-slate-600"} />
                                    <span className="font-mono text-white text-sm">{isBurningUI ? "BRAKING..." : "IDLE"}</span>
                                </div>
                    ...
                    <DashboardButton
                        ...
                        onClick={parkInOrbit}
                        disabled={phase === 'success' || phase === 'failed'}
                        className={velocity >= PHYSICS.MIN_SUCCESS_VEL && velocity <= PHYSICS.MAX_SUCCESS_VEL ? "animate-pulse" : "opacity-50"}
                    />
