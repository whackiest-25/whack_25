...
import {
    Camera, Thermometer, Database, Wifi, Battery, Sun,
    RotateCcw, CheckCircle, AlertTriangle, Globe, Zap,
    Server, Radio, Scan, Image as ImageIcon, XCircle, Activity, ArrowLeft,
    Info, Target, BookOpen, Medal, Lightbulb, AlertOctagon
} from 'lucide-react';
...
const MRD_CONTENT = {
    ...
    failure_tips: {
        "CRITICAL POWER FAILURE. MISSION LOST.": {
            ...
            analysis: "The spacecraft's battery dropped to 0%, causing a total systems failure. All instruments, communication systems, and attitude control are now offline. The mission cannot be recovered.",
            correction: "Monitor battery levels constantly. When below 30%, immediately point to SUN to recharge. Don't activate multiple instruments simultaneously when battery is low. Science collection can wait - power cannot."
        },
        "BUFFER OVERFLOW": {
            title: "DATA LOSS - STORAGE FULL",
            analysis: "The local data buffer reached maximum capacity (200 points) and had to discard incoming science data. This wastes valuable observation time.",
            correction: "Transmit data to Earth before buffer fills up. When buffer is near 150-180 points, switch to EARTH orientation and transmit. Better data management prevents loss."
        }
    },
    success: {
        title: "SCIENCE MISSION COMPLETE",
        mission_impact: "Outstanding operations! You've successfully collected and transmitted 1000 data points from Mars orbit. The data includes surface images, thermal maps, and methane measurements that will advance our understanding of the Red Planet.",
        details: [
            {
                topic: "MISSION DATA",
                content: "• Data Collected: 1000 science points\n• Instruments Used: MCC Camera, TIS Thermal, MSM Methane\n• Orbit Cycles: ~8-12 orbits\n• Mission Duration: ~10-15 hours (simulated)\n• Status: Mission success"
            },
            {
                topic: "SCIENTIFIC INSTRUMENTS",
                content: "• MCC: Mars Color Camera - captured surface images\n• TIS: Thermal Infrared Spectrometer - mapped heat signatures\n• MSM: Methane Sensor for Mars - detected atmospheric composition\n• HGA: High Gain Antenna - transmitted data to Earth"
            },
            {
                topic: "PERFORMANCE REPORT",
                content: "Your power management and data transmission strategy was excellent. Balancing charging, observation, and communication is the key to space mission success."
            },
            {
                topic: "MISSION COMPLETE",
                content: "Congratulations! You've successfully completed all 8 levels of the ISRO Mission Simulator. From rocket assembly to Mars science operations, you've experienced the full journey of India's Mangalyaan mission. This is a historic achievement!"
            }
        ]
    }
};
...
const RetroButton = ({ label, onClick, disabled, active, icon: Icon, color = "blue", className = "" }) => {
    const colors = {
    ...
    return (
        <div className="mb-2">
            <div className="flex justify-between text-[10px] font-pixel text-slate-400 mb-1">
                ...
                <span className={isLow ? "text-red-500 animate-pulse" : "text-white"}>{Math.floor(value)}/{max}</span>
            </div>
...
export default function Level8ScienceOps({ onBack, onNextLevel }) {
    ...
    const [activeInstruments, setActiveInstruments] = useState({ MCC: false, TIS: false, MSM: false });
    ...
    const updateGame = useCallback(() => {
        ...
        setBattery(bat => {
            ...
            else if (orientation === 'EARTH') change += 0.1; // Partial charge
    ...
    }, [gameActive, missionWon, missionFailed, orientation, activeInstruments, isTransmitting, buffer, isDaySide]);
                    ...
                    <div className="space-y-4">
                        <div className="bg-gradient-to-r from-green-950/60 to-blue-950/60 border-2 border-green-500/50 p-4 rounded-lg">
                            <div className="flex items-start gap-3 mb-3">
                                ...
                                <div>
                                    <h3 className="font-pixel text-sm text-green-300 mb-2">CONGRATULATIONS, COMMANDER!</h3>
                                    <p className="text-slate-200 text-xs leading-relaxed">
                                        You have successfully completed all phases of India's historic Mars Orbiter Mission (Mangalyaan).
                                        From assembly to science operations, you've experienced the entire journey of ISRO's first interplanetary mission.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-red-950/30 border border-red-700/50 p-3 rounded">
                            ...
                            <div className="text-slate-300 text-xs space-y-2">
                                <p>
                                    <strong className="text-white">Launch Date:</strong> November 5, 2013<br />
                                    <strong className="text-white">Mars Arrival:</strong> September 24, 2014<br />
                                    <strong className="text-white">Mission Cost:</strong> $74 million (₹450 crore)
                                </p>
                                <p className="text-amber-200 font-semibold">
                                    Mangalyaan made India the first Asian nation to reach Mars orbit, and the first nation
                                    in the world to succeed on their very first attempt!
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="bg-purple-950/30 border border-purple-700/50 p-3 rounded">
                            ...
                            <p className="text-slate-300 text-xs leading-relaxed">
                                Mangalyaan demonstrated India's space capabilities on the world stage and inspired millions
                                to pursue science and technology. It proved that ambitious space missions don't require
                                unlimited budgets - just brilliant engineering and determination.
                            </p>
                        </div>
            ...
            <div className="w-full max-w-5xl border-b-4 border-slate-700 pb-4 mb-6 flex justify-between items-end">
                ...
                <div className="flex items-end gap-4">
                    ...
                    <div className="text-right">
                        ...
                        <div className="font-mono text-xl text-green-400">{((score / GOAL_SCORE) * 100).toFixed(1)}%</div>
                    </div>
                </div>
            </div>
                    ...
                    <div className="relative aspect-video bg-black border-4 border-slate-700 rounded-xl overflow-hidden shadow-2xl group">
                        ...
                        <div className="absolute top-4 left-4 z-30 font-pixel text-[10px] text-slate-400">
                            <div className="bg-black/50 p-2 rounded border border-slate-600">
                                <div>LIGHT: {isDaySide ? <span className="text-yellow-400">DAY SIDE</span> : <span className="text-blue-400">NIGHT SIDE</span>}</div>
                                <div>MODE: <span className="text-white">{orientation} POINTING</span></div>
                            </div>
                        </div>
                        ...
                        `}>
                                    {orientation === 'MARS' && <Camera size={16} className="text-red-500 animate-pulse" />}
                                    {orientation === 'EARTH' && <Wifi size={16} className="text-blue-500 animate-pulse" />}
                                    {orientation === 'SUN' && <Sun size={16} className="text-yellow-500 animate-spin-slow" />}
                                ...
                                {orientation === 'MARS' && activeInstruments.MCC && isDaySide && (
                                    <div className="absolute -top-8 text-yellow-300 font-pixel text-[8px] animate-bounce">CAPTURING</div>
                                )}
                                {orientation === 'EARTH' && isTransmitting && (
                                    <div className="absolute -top-8 text-blue-300 font-pixel text-[8px] animate-pulse">UPLINKING</div>
                                )}
                            </div>
                        </div>
                            ...
                            {Object.entries(INSTRUMENTS).map(([key, inst]) => (
                                ...
                                `}
                                    >
                                        ...
                                        <div className={`w-2 h-2 rounded-full ${activeInstruments[key] ? "bg-green-400 shadow-[0_0_5px_lime]" : "bg-slate-600"}`} />
                                    </button>
                                    {/* Warnings */}
                                    {activeInstruments[key] && orientation !== 'MARS' && (
                                        <AlertTriangle size={16} className="text-amber-500 animate-pulse" title="Wrong Orientation" />
                                    )}
                                    {activeInstruments[key] && inst.needsLight && !isDaySide && orientation === 'MARS' && (
                                        <div className="text-[8px] font-pixel text-red-400 w-12 leading-tight">NEEDS LIGHT</div>
                                    )}
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="mt-4 p-2 bg-black/40 rounded border border-slate-700">
                            <div className="flex justify-between text-[9px] font-mono text-slate-400">
                                ...
                                <span className="text-red-400">
                                    -{orientation === 'MARS' ?
                                        Object.entries(activeInstruments).reduce((acc, [k, v]) => acc + (v ? INSTRUMENTS[k].cost : 0), 0).toFixed(1)
                                        : '0.0'} /tick
                                </span>
                            </div>
                        </div>
